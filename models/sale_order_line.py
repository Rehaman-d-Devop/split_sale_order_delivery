...
class SaleOrderLine(models.Model):
    ...
    f_planned_date = fields.Datetime(string="Planned Date")
    procurement_group_id = fields.Many2one("procurement.group", "Procurement group", copy=False)
    
    def _get_procurement_group(self):
        if self.order_id.f_delivery_split:
            line = self.search([
                ('order_id', '=', self.order_id.id),
                ('procurement_group_id', '!=', False), 
                ('f_planned_date', '=', self.f_planned_date)
            ], limit=1)
            if line.procurement_group_id:
                return line.procurement_group_id
            else:
                return False
        else:
    ...
    def _prepare_procurement_values(self, group_id=False):
        ...
        self.ensure_one()
        if self.order_id.f_delivery_split:
            values['date_planned'] = self.f_planned_date
            values['date_deadline'] = self.f_planned_date
        return values

    def write(self, vals):
        res = super().write(vals)
        moves_to_upd = set()
        if "f_planned_date" in vals:
            for move in self.move_ids:
                if move.state not in ["cancel", "done"]:
                    moves_to_upd.add(move.id)
        if moves_to_upd:
            self.env["stock.move"].browse(moves_to_upd).write(
                {"date_deadline": vals.get("f_planned_date")}
            )
        return res
